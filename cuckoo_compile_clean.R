...
dat<-read.csv("N:/cuckoo_tracking/data/cuckoos.csv", h=T)

head(dat)

#---- remove lines with blank location_dates ----
d1 <- dat[-which(is.na(dat$location_date)),]
...
strategy.dat <- read.csv("T:/Mark M/scripts/cuckoo migratory strategy and Sahara crossing success 2014_bird year multiples_NEW1.csv", header=T)
...
d2 <- merge(d2a, strategy.dat, by=c("ptt","year"))
