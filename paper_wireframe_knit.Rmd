...
Rather than looking at arrival of first rains we can concentrate on arrival of the monsoon, in particular its temporal and spatial variation. In the visualisation below we define monsoon arrival as the day of year when there has been cumulatively > 100ml of rain(we can modify this to any value) for each pixel. Turn of all the yearly tick boxes on the right then explore each year in turn to see temporal variation. Pan and zoom to explore spatial variation and how birds within each year time their stopovers to co-incide with arrival of monsoon. *Note how in 2016 rains are late across the entire region, but timing of departures from West Africa do not seem to be affected*

```{r, warning=F, message=F, echo=F} 
 
r2012_mon<-raster('C:/cuckoo_tracking/sourced_data/PERSIANN_rainfall/mons_arr2012.tif')
r2013_mon<-raster( 'C:/cuckoo_tracking/sourced_data/PERSIANN_rainfall/mons_arr2013.tif')
r2014_mon<-raster( 'C:/cuckoo_tracking/sourced_data/PERSIANN_rainfall/mons_arr2014.tif')
r2015_mon<-raster( 'C:/cuckoo_tracking/sourced_data/PERSIANN_rainfall/mons_arr2015.tif')
r2016_mon<-raster( 'C:/cuckoo_tracking/sourced_data/PERSIANN_rainfall/mons_arr2016.tif')
r2017_mon<-raster( 'C:/cuckoo_tracking/sourced_data/PERSIANN_rainfall/mons_arr2017.tif')
r2018_mon<-raster( 'C:/cuckoo_tracking/sourced_data/PERSIANN_rainfall/mons_arr2018.tif')


setwd('C:/cuckoo_tracking/sourced_data')

wrld<-readOGR(layer='TM_WORLD_BORDERS-0.3', dsn='country_borders', verbose = FALSE)


dat$year<-as.integer(dat$year)
 soversWA1<-left_join(soversWA, dat[,c(1,2,6)], by=c('ptt', 'year'))
 
 forsp<-na.omit(soversWA1[,c(1,2,7,8,9,17,18,22)])
...
pal <- colorBin(c('#9e0142','#d53e4f','#f46d43','#fdae61','#fee08b','#e6f598','#abdda4','#66c2a5','#3288bd','#5e4fa2'), c(NA, 0:125), na.color = "transparent")

 m <- leaflet() %>%
  addTiles() %>% 
   
  addRasterImage(r2012_mon, colors = pal, opacity = 0.8, group='2012') %>%
     addRasterImage(r2013_mon, colors = pal, opacity = 0.8, group='2013') %>%
     addRasterImage(r2014_mon, colors = pal, opacity = 0.8, group='2014') %>%
     addRasterImage(r2015_mon, colors = pal, opacity = 0.8, group='2015') %>%
     addRasterImage(r2016_mon, colors = pal, opacity = 0.8, group='2016') %>%
     addRasterImage(r2017_mon, colors = pal, opacity = 0.8, group='2017') %>%
     addRasterImage(r2018_mon, colors = pal, opacity = 0.8, group='2018') %>%
   addLegend(pal = pal, values = c(NA, 0:125),
    title = "Monsoon arrival day") %>%
   
   addPolylines(data=wrld[wrld$REGION==2,], color=1, weight=2, opacity=1) %>%
  
  
  addPolylines(data=dat_ls[dat_ls$year==2012,], weight=1, color=1, group='2012', dashArray='4,2') %>%

  addCircleMarkers(data=dat_sp[dat_sp$year==2012,], popup= paste("ptt:", dat_sp[dat_sp$year==2012,]$ptt, "<br>", "name:", dat_sp[dat_sp$year==2012,]$name,"<br>", "SO start:", dat_sp[dat_sp$year==2012,]$SO_startDOY,"<br>", "depart WA:", dat_sp[dat_sp$year==2012,]$DEPwestAF), color='#219607', stroke=T, radius=3, group='2012') %>%
   
     addPolylines(data=dat_ls[dat_ls$year==2013,], weight=1, color=1, group='2013', dashArray='4,2') %>%

  addCircleMarkers(data=dat_sp[dat_sp$year==2013,], popup= paste("ptt:", dat_sp[dat_sp$year==2013,]$ptt, "<br>", "name:", dat_sp[dat_sp$year==2013,]$name,"<br>", "SO start:", dat_sp[dat_sp$year==2013,]$SO_startDOY,"<br>", "depart WA:", dat_sp[dat_sp$year==2013,]$DEPwestAF), color='#219607', stroke=T, radius=3, group='2013') %>%
   
     addPolylines(data=dat_ls[dat_ls$year==2014,], weight=1, color=1, group='2014', dashArray='4,2') %>%

  addCircleMarkers(data=dat_sp[dat_sp$year==2014,], popup= paste("ptt:", dat_sp[dat_sp$year==2014,]$ptt, "<br>", "name:", dat_sp[dat_sp$year==2014,]$name,"<br>", "SO start:", dat_sp[dat_sp$year==2014,]$SO_startDOY,"<br>", "depart WA:", dat_sp[dat_sp$year==2014,]$DEPwestAF), color='#219607', stroke=T, radius=3, group='2014') %>%
   
     addPolylines(data=dat_ls[dat_ls$year==2015,], weight=1, color=1, group='2015', dashArray='4,2') %>%

  addCircleMarkers(data=dat_sp[dat_sp$year==2015,], popup= paste("ptt:", dat_sp[dat_sp$year==2015,]$ptt, "<br>", "name:", dat_sp[dat_sp$year==2015,]$name,"<br>", "SO start:", dat_sp[dat_sp$year==2015,]$SO_startDOY,"<br>", "depart WA:", dat_sp[dat_sp$year==2015,]$DEPwestAF), color='#219607', stroke=T, radius=3, group='2015') %>%
   
     addPolylines(data=dat_ls[dat_ls$year==2016,], weight=1, color=1, group='2016', dashArray='4,2') %>%

  addCircleMarkers(data=dat_sp[dat_sp$year==2016,], popup= paste("ptt:", dat_sp[dat_sp$year==2016,]$ptt, "<br>", "name:", dat_sp[dat_sp$year==2016,]$name,"<br>", "SO start:", dat_sp[dat_sp$year==2016,]$SO_startDOY,"<br>", "depart WA:", dat_sp[dat_sp$year==2016,]$DEPwestAF), color='#219607', stroke=T, radius=3, group='2016') %>%
   
     addPolylines(data=dat_ls[dat_ls$year==2017,], weight=1, color=1, group='2017', dashArray='4,2') %>%

  addCircleMarkers(data=dat_sp[dat_sp$year==2017,], popup= paste("ptt:", dat_sp[dat_sp$year==2017,]$ptt, "<br>", "name:", dat_sp[dat_sp$year==2017,]$name,"<br>", "SO start:", dat_sp[dat_sp$year==2017,]$SO_startDOY,"<br>", "depart WA:", dat_sp[dat_sp$year==2017,]$DEPwestAF), color='#219607', stroke=T, radius=3, group='2017') %>%
   
     addPolylines(data=dat_ls[dat_ls$year==2018,], weight=1, color=1, group='2018', dashArray='4,2') %>%

  addCircleMarkers(data=dat_sp[dat_sp$year==2018,], popup= paste("ptt:", dat_sp[dat_sp$year==2018,]$ptt, "<br>", "name:", dat_sp[dat_sp$year==2018,]$name,"<br>", "SO start:", dat_sp[dat_sp$year==2018,]$SO_startDOY,"<br>", "depart WA:", dat_sp[dat_sp$year==2018,]$DEPwestAF), color='#219607', stroke=T, radius=3, group='2018') %>%
   
   
     addLayersControl(overlayGroups = c('2012', '2013', '2014', '2015', '2016', '2017', '2018'), options=layersControlOptions(collapsed=F))
   
m

```

OK so are there any explanatory environmental variables that can explain the variation in cuckoo departure from West Africa

```{r, warning=F, message=F, echo=F} 
 # make two gradient metrics
 dmod$ndvi_last_ang<-dmod$ndviTdiff_last*dmod$ndvidiff_last
dmod$rain_first_ang<-(dmod$monsTdiff_first+25)*dmod$raindiff2_first
 m0<-lmer(DEPwestAF~(1|ptt)+(1|year), data=dmod[dmod$DEPwestAF %in% 85:120,])
m1<-lmer(DEPwestAF~dep_country+(1|ptt)+(1|year), data=dmod[dmod$DEPwestAF %in% 85:120,])
m2<-lmer(DEPwestAF~firstCdepartureG+(1|ptt)+(1|year), data=dmod[dmod$DEPwestAF %in% 85:120,])
m3<-lmer(DEPwestAF~departureGarr+(1|ptt)+(1|year), data=dmod[dmod$DEPwestAF %in% 85:120,])
m4<-lmer(DEPwestAF~nSOdepartureG+(1|ptt)+(1|year), data=dmod[dmod$DEPwestAF %in% 85:120,])
m5<-lmer(DEPwestAF~monsTdiff_first+(1|ptt)+(1|year), data=dmod[dmod$DEPwestAF %in% 85:120,])
m6<-lmer(DEPwestAF~raindiff2_first+(1|ptt)+(1|year), data=dmod[dmod$DEPwestAF %in% 85:120,])
m7<-lmer(DEPwestAF~emodisNDVI_last+(1|ptt)+(1|year), data=dmod[dmod$DEPwestAF %in% 85:120,])
m8<-lmer(DEPwestAF~ndviTdiff_last+(1|ptt)+(1|year), data=dmod[dmod$DEPwestAF %in% 85:120,])
m9<-lmer(DEPwestAF~ndvidiff_last+(1|ptt)+(1|year), data=dmod[dmod$DEPwestAF %in% 85:120,])
m10<-lmer(DEPwestAF~emodisANOM_mean+(1|ptt)+(1|year), data=dmod[dmod$DEPwestAF %in% 85:120,])
m11<-lmer(DEPwestAF~Trees_mean+(1|ptt)+(1|year), data=dmod[dmod$DEPwestAF %in% 85:120,])
m12<-lmer(DEPwestAF~Croplands_mean+(1|ptt)+(1|year), data=dmod[dmod$DEPwestAF %in% 85:120,])
m13<-lmer(DEPwestAF~d_river_mean+(1|ptt)+(1|year), data=dmod[dmod$DEPwestAF %in% 85:120,])
 m14<-lmer(DEPwestAF~rain_first_ang+(1|ptt)+(1|year), data=dmod[dmod$DEPwestAF %in% 85:120,])
 m15<-lmer(DEPwestAF~ndvi_last_ang+(1|ptt)+(1|year), data=dmod[dmod$DEPwestAF %in% 85:120,])
 anova(m0,m1,m2,m3,m4,m5,m6,m7,m8,m9,m10, m11, m12,m13, m14, m15)
 moddat<-dmod[dmod$DEPwestAF %in% 85:120,]
 mfin<-lmer(DEPwestAF~departureGarr+nSOdepartureG+monsTdiff_first*raindiff2_first+emodisNDVI_last+ndviTdiff_last*ndvidiff_last+emodisANOM_mean+Trees_mean+Croplands_mean+d_river_mean+(1|ptt)+(1|year), data=moddat,  na.action = na.fail)
 mdr<-dredge(mfin)
 get.models(mdr, subset = delta < 4, method = "REML")
 #qplot(data=dmod[dmod$DEPwestAF %in% 85:120,], y=DEPwestAF, x=nSOdepartureG)
 #qplot(data=dmod[dmod$DEPwestAF %in% 85:120,], y=DEPwestAF, x=ndviTdiff_last)
 #qplot(data=dmod[dmod$DEPwestAF %in% 85:120 & dmod$emodisANOM_mean>-0.05,], y=DEPwestAF, x=emodisANOM_mean)
```
