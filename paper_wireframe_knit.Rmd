...
```
 ...
 Habitat?? - test each category against dep from west africa
 

```{r, warning=F, message=F, echo=F} 
ggplot(data=dat, aes(x=dur_finalSO, y=DEPwestAF ))+xlab("Departure country")+ylab("Departure date West Africa (DOY)")+geom_point(shape=1)+theme_bw() 
...
m <- leaflet() %>%
  addTiles() %>% 
  addMarkers(data = dat_sp, popup= paste("ptt:", dat_sp$ptt, "<br>", "year:", dat_sp$year,"<br>", "Departure date:", dat_sp$DEPwestAF))
